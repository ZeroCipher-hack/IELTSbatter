// node_modules/@met4citizen/headtts/modules/utils.mjs
function isModuleWebWorkers() {
  let support = false;
  try {
    const tester = {
      get type() {
...
function trace(...outputs) {
  const s = "HeadTTS [" + (/* @__PURE__ */ new Date()).toISOString().slice(11, 23) + "] ";
...
var b64Lookup = typeof Uint8Array === "undefined" ? [] : new Uint8Array(256);
for (let i = 0; i < b64Chars.length; i++) b64Lookup[b64Chars.charCodeAt(i)] = i;
function b64ToArrayBuffer(chunk) {
...
// node_modules/@met4citizen/headtts/modules/headtts.mjs
var HeadTTS = class {
  /**
  * @constructor
  ...
  */
  emit(title, message, handlers, isThrowable = false, firstOnly = true) {
    const isTraceEvents = this.settings.trace & traceMask.events;
    if (isTraceEvents) {
      trace("EMIT: " + title, message);
    }
    let isCalled = false;
  ...
  */
  async connect(settings = null, onprogress = null, onerror = null) {
    const isTraceConnection = this.settings.trace & traceMask.connect;
    const isTraceMessages = this.settings.trace & traceMask.messages;
    if (settings === null) {
    ...
    for (let i = 0; i < this.settings.endpoints.length && !this.isConnected; i++) {
      ...
      if (endpointLowerCase === "webgpu" || endpointLowerCase === "wasm") {
        if (!isModuleWebWorkers()) {
          if (isTraceConnection) {
            trace("Your browser doesn't support Module Web Workers.");
          }
          connectionErrorLog.push("Your browser doesn't support Module Web Workers.");
          continue;
        }
        if (endpointLowerCase === "webgpu" && !isWebGPU()) {
          if (isTraceConnection) {
            trace("Your browser doesn't support WebGPU.");
          }
        ...
        try {
          await new Promise((resolve, reject) => {
            ...
            let timeout = setTimeout(rejectedTimeout, 3e4);
            this.ww.onmessage = (ev) => {
              if (ev.data?.type === "progress") {
                if (timeout) {
                  clearTimeout(timeout);
                  timeout = setTimeout(rejectedTimeout, 1e4);
                }
            ...
            if (isTraceMessages) {
              trace("OUT: HeadTTS -> worker, message=", message);
            }
            this.ww.postMessage(message);
          });
          this.ww.onmessage = (ev) => {
            if (isTraceMessages) {
              trace("IN: worker -> HeadTTS, message=", ev.data);
            }
          ...
          if (isTraceConnection) {
            trace("Failed to start Module Web Worker.");
          }
        ...
        if (url && (url.protocol === "ws:" || url.protocol === "wss:")) {
          try {
            await new Promise((resolve, reject) => {
              const rejectedTimeout = () => reject("Connection timed out.");
              let timeout = setTimeout(rejectedTimeout, 5e3);
            ...
            this.ws.onmessage = async (ev) => {
              if (isTraceMessages) {
                trace("IN: WebSocket -> HeadTTS, message=", ev.data);
              }
              this.processData(ev.data);
            };
            ...
            this.ws.onclose = (ev) => {
              ...
              if (ev.wasClean) {
                if (isTraceConnection) {
                  trace("WebSocket connection closed cleanly, event=", ev);
                }
              } else {
            ...
            if (isTraceConnection) {
              trace('Failed to connect to "' + endpoint + "'.");
            }
          ...
          try {
            ...
            console.log(error);
            if (isTraceMessages) {
              trace('The request hello failed on "' + endpoint + '".');
            }
  ...
  */
  async setup(data, onerror = null) {
    const isTraceMessages = this.settings.trace & traceMask.messages;
    if (isTraceMessages) {
      trace("SETUP: APP -> HeadTTS, data=", data);
    }
    const events = [onerror, this.onerror];
  ...
  */
  async synthesize(data, onmessage = null, onerror = null) {
    if (this.settings.traceLevel) {
      trace(this.settings.traceLevel, 3, "SYNTHESIZE: APP -> HeadTTS, data=", data);
    }
    const events = [onerror, this.onerror];
    if (!data) this.emit("error", new Error("Data not set."), events, true);
  ...
  */
  async custom(data, onmessage = null, onerror = null) {
    const isTraceMessages = this.settings.trace & traceMask.messages;
    if (isTraceMessages) {
      trace("CUSTOM: APP -> HeadTTS, data=", data);
    }
    const events = [onerror, this.onerror];
    if (!data) this.emit("error", new Error("Data not set."), events, true);
    const item = {
  ...
  async processIn(onerror = null) {
    ...
    while (this.queueIn.length) {
      ...
      if (item.message.type === "setup") {
        ...
        if (this.ws) {
          if (isTraceMessages) {
            trace("OUT: HeadTTS -> WebSocket, message=", item.message);
          }
        ...
        if (this.ws) {
          ...
          if (isTraceMessages) {
            trace("OUT: HeadTTS -> worker, message=", item.message);
          }
          ...
          if (item.message.type === "synthesize") {
            ...
            if (isTraceMessages) {
              trace("OUT: HeadTTS -> REST, message=", item.message.data);
            }
            ...
            if (item.message.data) {
              ...
              if (itemdata.audio) {
                if (typeof itemdata.audio === "string") {
                  itemdata.audio = b64ToArrayBuffer(itemdata.audio);
                }
                if (itemdata.audioEncoding === "wav") {
  ...
  */
  async processOut() {
    if (this.isProcessingOut) return;
...
/*! For license information please see headtts.bundle.mjs.LEGAL.txt */
